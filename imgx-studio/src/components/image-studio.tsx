...
import { extractGeneratedImages, getImageApiError, type GeneratedImage } from "@/lib/image-request"
import {
  DEFAULT_LOCALE,
  LOCALE_COOKIE_KEY,
  LOCALE_OPTIONS,
...
import { buildImgXSub2APIConfig, loadImgXSub2APIKey } from "@/lib/sub2api-integration"
...
export function ImageStudio({ initialLocale = DEFAULT_LOCALE }: { initialLocale?: Locale }) {
  ...
  const [embeddedSub2API, setEmbeddedSub2API] = useState(false)
  ...
  useEffect(() => {
    const config = buildImgXSub2APIConfig()
    if (config.enabled) {
      clearStoredConnectionPreferences()
      window.setTimeout(() => setEmbeddedSub2API(true), 0)
      void loadImgXSub2APIKey(config)
        .then(({ apiKey: injectedKey, baseUrl }) => {
          setApiKey(injectedKey)
          setEndpoint(baseUrl)
          setRememberKey(false)
          setHasLoadedPreferences(true)
        })
        .catch((error) => {
          toast.error(error instanceof Error ? error.message : "Sub2API 生图密钥加载失败")
          setHasLoadedPreferences(true)
        })
      return
    }
  ...
  }, [apiKey, embeddedSub2API, endpoint, hasLoadedPreferences, rememberKey])
  ...
  async function callProxy(requestedCount: number): Promise<{ endpoint: string; images: GeneratedImage[] }> {
    ...
    const requestUrl = `${endpoint.replace(/\/+$/, "")}/images/${inputUploads.length ? "edits" : "generations"}`
    let response: Response
    if (embeddedSub2API) {
      const headers = { Authorization: `Bearer ${apiKey.trim()}` }
      if (inputUploads.length) {
        const directForm = new FormData()
        directForm.append("background", background)
        directForm.append("model", model)
        directForm.append("n", String(requestedCount))
        directForm.append("output_format", outputFormat)
        directForm.append("prompt", requestPrompt)
        directForm.append("quality", quality)
        directForm.append("size", size)
        for (const upload of inputUploads) directForm.append("image", upload.file, upload.file.name)
        response = await fetch(requestUrl, { method: "POST", body: directForm, headers })
      } else {
      ...
      response = await fetch("/api/images", { method: "POST", body: formData })
    }
    ...
    if (!response.ok) {
      throw new Error(getImageApiError(payload) || payload.error || t(locale, "requestFailedStatus", { status: response.status }))
    }

    const images = embeddedSub2API ? extractGeneratedImages(payload, outputFormat) : payload.images ?? []
    if (!images.length) {
      throw new Error(t(locale, "noImageInPayload"))
    }

    return {
      endpoint: payload.endpoint || requestUrl,
      images,
    }
  }
