...
import { getSub2APIBootstrapConfig, loadSub2APIProfile } from './lib/sub2apiIntegration'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const sub2apiConfig = getSub2APIBootstrapConfig()
    ...
    if (sub2apiConfig.enabled) {
      void loadSub2APIProfile(sub2apiConfig)
        .then(async (profile) => {
          await initStore()
        ...
        .catch(async (error) => {
          await initStore()
          useStore.getState().showToast(error instanceof Error ? error.message : 'Sub2API 生图工作台初始化失败。', 'error')
        })
      return
    }
